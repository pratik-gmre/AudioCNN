export const Waveform = ({data,title}:{data:number[],title:string}) => {
    ...
    if(validData.length === 0) return null
        ...
        const pathData =  validData.map((sample,i)=> {
            ...
            if (range > 0 ){
                const normalizedSample = (sample - min)/range;   //make value of sample 0-1
    ...
    return(
        <div className="flex h-full w-full flex-col">
            ...
            <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet" className="block max-w-full rounded border border-stone-200 max-h-[300px]">
